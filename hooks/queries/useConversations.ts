...
export function useConversationList(walletAddress: string | undefined) {
  return useQuery({
    ...
    placeholderData: () => {
      ...
      const cached = storage.getString(`chat:list:${walletAddress}`)
...
export function useConversation(id: string | null, walletAddress: string | undefined) {
  return useQuery({
    ...
    placeholderData: () => {
      ...
      const cached = storage.getString(`chat:conv:${id}`)
      if (!cached) return undefined
      ...
      return JSON.parse(cached) as unknown as import("@/api/conversations.types").ConversationDetailResponse
    },
  })
}
...
export function useDeleteConversation() {
  ...
  return useMutation({
    ...
    onMutate: async ({ id, walletAddress }) => {
      await queryClient.cancelQueries({ queryKey: ["conversations", walletAddress] })
      ...
      const mmkvKey = `chat:list:${walletAddress}`
    ...
    onError: (_err, { walletAddress }, ctx) => {
      if (ctx?.previous) {
        queryClient.setQueryData(["conversations", walletAddress], ctx.previous)
      }
...
export function useRenameConversation() {
  ...
  return useMutation({
    ...
    onMutate: async ({ id, walletAddress, title }) => {
      ...
      const convRaw = storage.getString(`chat:conv:${id}`)
      if (convRaw) {
        const conv = JSON.parse(convRaw) as ConversationCache
        conv.title = title
        storage.set(`chat:conv:${id}`, JSON.stringify(conv))
      }
