...
function getCached(key: string): ENSResolution | null {
  ...
  const row = database.getFirstSync<{
    name: string | null; address: string | null; avatar: string | null;
    text_records: string | null; contenthash: string | null; cached_at: number;
  }>("SELECT * FROM ens_cache WHERE key = ?", [key]);

  if (!row || Date.now() - row.cached_at > ENS_TTL) return null;

  return {
    name: row.name, address: row.address, avatar: row.avatar ?? undefined,
    textRecords: row.text_records ? JSON.parse(row.text_records) : undefined,
    contenthash: row.contenthash ?? undefined, chainId: 1,
  };
}
...
function setCache(key: string, resolution: ENSResolution): void {
  ...
  database.runSync(
    ...
    [key, resolution.name, resolution.address, resolution.avatar ?? null,
     resolution.textRecords ? JSON.stringify(resolution.textRecords) : null,
     resolution.contenthash ?? null, Date.now()],
  );
}

export async function resolveForward(name: string): Promise<ENSResolution | null> {
  ...
  try {
    ...
    const resolution: ENSResolution = { name: normalizedName, address, chainId: 1 };
...
export async function resolveReverse(address: string): Promise<ENSResolution | null> {
...
export async function resolveTextRecords(name: string, keys: string[]): Promise<Record<string, string>> {
  const client = getPublicClient(mainnet);
