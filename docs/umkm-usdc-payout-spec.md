...
- **Q1 — Arc mainnet ID & Gateway availability.** ✅ Resolved — no longer a blocking question. Arc is Circle domain `26`, first-class in Gateway testnet since Feb 2026, and Arc is EVM-compatible so our `EvmWalletKit` adapter handles it without a subclass. When Arc mainnet launches, it appears in `GET /v1/info` automatically; rollout is an `EXPO_PUBLIC_ARC_CHAIN_ID` env flip per §10.1. No code change, no spec change, no decision required — the chain-agnostic architecture (§5.5) means new EVM chains are config, not engineering.
